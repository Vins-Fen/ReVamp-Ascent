...
import dataAccess.ConPool;
import dataAccess.JavaBeans.Ordine;
    ...
    public Map<Integer, List<Map<String, Object>>> doRetrieveStoricoUtente(String nomeUtente) {
     ...
     WITH righe AS (
                                                   SELECT o.id_ordine, o.data,
                                                          ...
                                                          c.quantita, c.prezzo_unitario,
                                                          COALESCE(c.sconto, 0) AS sconto
                                                   FROM ordine o
                                               ...
                                               totali AS (
                                                   SELECT id_ordine,
                                                          ...
                                                          SUM(prezzo_unitario * (1 - COALESCE(sconto, 0)) * quantita) AS importo_totale
                                                   FROM righe
                                                   GROUP BY id_ordine
                                               ),
                                               img AS (
                                                   SELECT ia.codice_articolo, MIN(ia.url) AS url
                                                   FROM immaginearticolo ia
                                                   WHERE ia.is_principale = TRUE
                                                   GROUP BY ia.codice_articolo
                                               )
                                               SELECT r.id_ordine, r.data,
                                                      r.nome_articolo, r.codice,
                                                      r.quantita, r.prezzo_unitario, r.sconto,
                                                      COALESCE(img.url, 'assets/img/default.jpg') AS immagine,
                                                      t.num_articoli, t.importo_totale
                                               FROM righe r
                                               JOIN totali t ON t.id_ordine = r.id_ordine
                                               LEFT JOIN img ON img.codice_articolo = r.codice
                                               ORDER BY r.id_ordine DESC, r.codice ASC;
    """;
